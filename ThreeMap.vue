<template>
    <div class="map-container" ref="rootContainerRef">
        <!-- 3D地图容器 -->
...
const props = withDefaults(defineProps<{
    ...
    mapConfig?: {
        ...
        blockHeight?: number;
        // 区块缩放倍率 示例：1
        blockScaleFactor?: number;
        // geojson数据类型 1-常规 2-天地图
        geojsonType?: number;
        // 中心底部光圈1
        circle1Data?: {
            // 光圈1透明度 示例：1
        ...
        lightPointBeamData?: {
            ...
            height: number;
            // 光柱颜色 示例：#00ffff
            lightPillarColor: string;
            // 标记点缩放系数 示例：0.15
            pointScaleFactor: number;
            // 标记点颜色 示例：#00ffff
            bottomMeshColor: string;
            // 光圈缩放系数 示例：0.3
            lightHaloScaleFactor: number;
            // 光圈颜色 示例：#00ffff
...
const rootContainerRef = ref<HTMLElement | null>(null);//外层容器的 ref
...
/** GeoJSON数据转换工具 */
const { transformGeoJSON, projectionRef, mapSizeRef } = useGeoJSONTransform();
/** 创建点位组 */
const { createPointsGroup } = useMapPoints(projectionRef, mapSizeRef);

// ==================== 全屏逻辑 (新增) ====================
// 直接传入 mapContainerRef，这样全屏时只放大地图容器
const { isFullscreen, toggle: toggleFullscreen } = useFullscreen(rootContainerRef);
...
class CurrentEarth extends BaseEarth {
    ...
    pointsGroup?: THREE.Group; // 用于存放点位的组
    ...
    async updateMap(data: any) {
        ...
        // 如果有点位数据，渲染点位 (确保在initModel之后，因为需要mapGroup存在)
        if (props.pointData) {
            this.updatePoints(props.pointData, props.mapConfig?.blockHeight);
     ...
     * 从GeoJSON数据创建3D地图模型
     */
    async initModel(originalData?: any) {
        if (!originalData) return;
        try {
            // 创建地图组，用于管理所有地图相关的3D对象
            this.mapGroup = new THREE.Group();

            const data = props.mapConfig?.geojsonType === 2 ? transformGeoJSON(originalData, {
                mapSize: props.mapConfig?.blockScaleFactor || 1,
            }) : originalData;
    ...
    updatePoints(data: PointItem[], mapHeight: number = 0.15) {
        ...
        if (this.pointsGroup) {
            ...
            const parent = this.pointsGroup.parent;
            if (parent) parent.remove(this.pointsGroup);

            // 内存清理
            this.pointsGroup.traverse((obj: any) => {
                if (obj.isSprite || obj.isMesh) {
                    if (obj.material) obj.material.dispose();
                    if (obj.geometry) obj.geometry.dispose();
                }
            });
            this.pointsGroup = undefined;
        }
        ...
        // 3. 添加到场景
        if (this.mapGroup) {
            this.mapGroup.add(this.pointsGroup);
        } else {
...
onMounted(async () => {
    ...
    try {
        ...
        if (props.mapData) {
            ...
            emit('map-ready');
            emit('map-ready', earthInstance);
        } else {
            errorMessage.value = '地图数据加载失败';
            isLoading.value = false;
    ...
    if (baseEarthRef.value && val) {
        ...
        try {
            ...
            if (val.mapData) {
                ...
                isLoading.value = false;
                emit('load-finish');
                emit('map-ready', baseEarthRef.value); // 数据更新后同样触发
            } else {
                errorMessage.value = '地图数据加载失败';
                isLoading.value = false;
                emit('render-error', errorMessage.value);
