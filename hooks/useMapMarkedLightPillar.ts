...
import { deepMerge, random } from '../util/common'
import label from '../assets/label.png';
import labelCircle from '../assets/labelCircle.png';
import beamLight from '../assets/beamLight.png'
 ...
 *  scaleFactor:1 整体缩放系数，用来调整标记点和光圈的缩放大小
 * }
 *
 * @returns
 */
export default function useMarkedLightPillar(options: any) {
    // 默认参数
    let defaultOptions = {
        pointTextureUrl: label,
        lightHaloTextureUrl: labelCircle,
        lightPillarUrl: beamLight,
        scaleFactor: 1, // 整体缩放系数
    }
    ...
    // 对象属性
    let getBoundingClientRect: any = null
    /**
     * 创建标记点
     * @param {number} pointScaleFactor 标记点缩放系数
     * @returns
     */
    const createPointMesh = (pointScaleFactor: number, color: string) => {
        // 标记点：几何体，材质，
        ...
        mesh.renderOrder = 10;
        // 缩放
        const scale = pointScaleFactor * defaultOptions.scaleFactor
        mesh.scale.set(scale, scale, scale)
        return mesh
    }
    /**
     * 创建光圈
     * @param {number} lightHaloScaleFactor 光圈缩放系数
     * @returns
     */
    const createLightHalo = (lightHaloScaleFactor: number, lightHaloColor: string) => {
        // 标记点：几何体，材质，
        const geometry = new THREE.PlaneGeometry(0.8, 0.8)
        const material = new THREE.MeshBasicMaterial({
            map: textureLoader.load(defaultOptions.lightHaloTextureUrl),
            color: lightHaloColor || 0x00ffff,
            side: THREE.DoubleSide,
            opacity: 0,
            transparent: true,
            depthWrite: false, //禁止写入深度缓冲区数据
        ...
        mesh.renderOrder = 10;
        // 缩放
        const scale = lightHaloScaleFactor * defaultOptions.scaleFactor
        mesh.scale.set(scale, scale, scale)
        // 动画延迟时间
     ...
     * @param {*} color
     */
    const setLightPillarColor = (group: any, color: number) => {
     ...
     * @returns
     */
    const getRaycasterObj = (event: any, container: any, camera: any, mesh: any) => {
        //屏幕坐标转WebGL标准设备坐标
     ...
     * @returns 返回选择的对象
     */
    const chooseLightPillar = (event: any, container: any, camera: any, mesh: any) => {
