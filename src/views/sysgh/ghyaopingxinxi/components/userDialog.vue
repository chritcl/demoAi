...
const initFormData = async () => {
  const res = await requestUtil.post("sys/ghmedicine/search_ghs_yg")
...
const handleConfirm = () => {
  if (yg_form_value.value === null) {
    ...
    if (value.value !== "") {
      requestUtil.post("sys/ghmedicine/updateghs_yg/" + value.value + "/" + ghs_form_value.value + "/" + yg_form_value.value).then(result => {
