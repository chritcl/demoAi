<template>
  <div class="app-container">
    <el-row :gutter="18" class="header">
      <el-col :span="7">
        <el-input placeholder="请输入药品名称..." v-model="queryForm.query"></el-input>
      </el-col>
      ...
      <el-button type="success" :icon="DocumentAdd" @click="handleDialogValue()" v-if="currentUser.roles==='供货商'">新增</el-button>
      <el-popconfirm title="您确定批量删除这些记录吗？" @confirm="handleDelete(null)">
        <template #reference>
          <el-button type="danger" :disabled="delBtnStatus" :icon="Delete" v-if="currentUser.roles==='超级管理员'||'供货商'">批量删除</el-button>
        </template>
      </el-popconfirm>
    </el-row>
    <el-table
...
<!--          <img :src="getServerUrl()+'image/medicinepic/'+scope.row.picture" width="110"/>-->
      ...
      <el-table-column prop="action" label="操作" width="300" fixed="right" align="center">
        <template v-slot="scope">
          ...
          <el-button style="margin-bottom: 0.4rem" color="#626aef" :icon="ShoppingBag" @click="handleCaigouDialog(scope.row.id,scope.row.gonghuoshangid)" v-if="currentUser.roles!=='供货商'">采购</el-button>
          <el-popconfirm v-if="scope.row.username!='admin'" title="您确定要删除这条记录吗？" @confirm="handleDelete(scope.row.id)">
            <template #reference>
              <el-button style="margin-bottom: 0.4rem" type="danger" :icon="Delete" v-if="currentUser.roles==='超级管理员'||'供货商'">删除</el-button>
            </template>
          </el-popconfirm>
        </template>
      </el-table-column>
  ...
  <Caigoudialog :id="dialogCaigouid" :userid="currentUser.id" :ghsid="dialogGhsid" @initMedicineList="initMedicineList" v-model="dialogCaigouVisible" :dialogCaigouVisible="dialogCaigouVisible"/>
</template>

<script setup>
import {Search, Refresh, View, Edit, Delete, Tools, DocumentAdd,ShoppingBag } from "@element-plus/icons-vue"
import {ref} from 'vue'
import requestUtil, {getServerUrl} from "@/utils/request"
import Dialog from "./components/dialog"
import { ElMessage } from 'element-plus'
import UserDialog from "./components/userDialog"
import Caigoudialog from "./components/caigoudialog"
...
const initMedicineList = async () => {
  if(currentUser.value.roles==='供货商'){
    const res = await requestUtil.post("sys/ghmedicine/listgh/"+currentUser.value.id, queryForm.value)
    ...
    const res = await requestUtil.post("sys/ghmedicine/list", queryForm.value)
    loading_table.value = true
    setTimeout(() => {
      tableData.value = res.data.yaopinxinxiList
      total.value = res.data.total
...
const handleDelete=async (id)=>{
  ...
  const res=await requestUtil.post("sys/ghmedicine/delete",ids)
 ...
 * 采购对话框
 * */
const dialogCaigouVisible=ref(false)
const dialogCaigouid=ref(-1)
const dialogGhsid=ref(-1)

const handleCaigouDialog=(id,ghsid)=>{
  dialogCaigouid.value=id
  dialogGhsid.value=ghsid
  dialogCaigouVisible.value=true
}
