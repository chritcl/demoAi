...
const useCoord = () => {
    ...
    const geoMercatorCoord = (longitude: number, latitude: number) => {
        var E = longitude
        var N = latitude
     ...
     * @returns
     */
    const geoSphereCoord = (R: number, longitude: number, latitude: number) => {
     ...
     * @returns
     */
    const getBoundingBox = (group: THREE.Group) => {
        // 包围盒计算模型对象的大小和位置
     ...
     * @returns
     */
    const setMeshQuaternion = (mesh: THREE.Mesh, R: number, lon: number, lat: number) => {
