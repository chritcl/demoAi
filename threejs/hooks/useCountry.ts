...
const useCountryLine = () => {
     ...
     * @returns
     */
    const createCountryFlatLine = (data: any, materialOptions = {}, lineType = 'LineLoop', resolution?: THREE.Vector2) => {
        let materialOpt = {
            color: 0x00ffff,
        };
        ...
        if (lineType === 'Line2') {
            material = new LineMaterial(materialOpt) as any;
        }
        let features = data.features;
        let lineGroup = new THREE.Group();
        for (let i = 0; i < features.length; i++) {
            const element = features[i];
            element.geometry.coordinates.forEach((coords: any, idx: number) => {
                // 每一块的点数据
                const points: any[] = [];
                if (lineType === 'Line2') {
                    ...
                    if (coords[0].length > 0) {
                        ...
                        if (firstPoint[0] !== lastPoint[0] || firstPoint[1] !== lastPoint[1]) {
                            // 如果不同，添加首点到末尾以闭合线条
                            points.push(firstPoint[0], firstPoint[1], 0);
                        }
                    }
     ...
     * @returns
     */
    const createLine = (points: any, material: any, lineType = 'LineLoop') => {
        let line = null;
        if (lineType === 'Line2') {
            ...
            line = new (THREE as any)[lineType](geometry, material);
